...
function Register(): JSX.Element {
  ...
  const [inviteBasedLogin, setInviteBasedLogin] = React.useState<boolean>(false);
  ...
  const loginWithGoogle = async () => {
    ...
    try {
      ...
      if (error.code === statusCodes.SIGN_IN_CANCELLED) {
        dispatch(setCustomerState({ uiState: FAILED, error: 'Sign Up Cancelelled' }));
    ...
    dispatch(registerCustomerAsync({ ...payload }))
      ...
      .catch((err) => {
        dispatch(
          setCustomerState({
            uiState: FAILED,
            error: 'Something went wrong while creating profile. Please try again.',
          })
        );
      });
  };
    ...
    <AppSafeAreaView loading={uiState === IN_PROGRESS}>
      <KeyboardAwareScrollView enableOnAndroid>
        ...
        <Flex flexDirection="column" flex={1} paddingX={5} mt={10}>
          ...
          <GradientButton text="CREATE ACCOUNT" onPress={handleSubmit(onSubmit)} />
          {!inviteBasedLogin && (
            <>
              <Spacer top={20} />
              <SocialLogin
