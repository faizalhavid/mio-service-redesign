...
export function Reschedule({
  ...
  const { member: orderDetail, uiState: orderDetailUiState } = useAppSelector(selectOrderDetails);
  const { uiState: rescheduleOrderUiState } = useAppSelector(selectRescheduleOrder);
  ...
  useEffect(() => {
    if (dt) {
      ...
      setReadableDateTime(`${dateTime.month} ${dateTime.date}, ${dateTime.slot}`);
    }
  }, [dt]);

  return (
    <Actionsheet isOpen={isOpen} onClose={() => setOpen(false)} hideDragIndicator>
      <Actionsheet.Content
        style={{
      ...
      >
        <VStack pt={15} bg="white" width="100%">
          <Center>
            ...
            <Text fontSize={14} color={AppColors.SECONDARY} fontWeight="semibold">
