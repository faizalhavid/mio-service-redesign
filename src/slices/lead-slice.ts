...
export const leadSlice = createAsyncSlice({
  ...
  reducers: {
    ...
    updateLeadState: (
      state,
      { payload }: PayloadAction<{ lead: LeadDetails }>
    ) => {
      state.member = { ...state.member, ...payload.lead };
    },
    updateLeadSubOrderState: (
      state,
      { payload }: PayloadAction<{ subOrder: SubOrder }>
    ) => {
      let _subOrders: SubOrder[] = [...state.member.subOrders];
...
// Actions
export const { resetLeadState, updateLeadState, updateLeadSubOrderState } =
  leadSlice.actions;
// Selectors
